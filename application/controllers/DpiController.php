	...
	public function guardarCambios()
	{
		...
		if (!empty($nombres) && is_array($nombres)) {
			foreach ($nombres as $index => $nombre) {
				...
				if (
					!empty($nombre) && !empty($tipos[$index]) && !empty($lugares_trabajo[$index]) &&
		...
		for ($i = 0; $i < count($nombre_empresa); $i++) {
			if (!empty($nombre_empresa[$i])) { // Verificamos que el nombre de la empresa no esté vacío
				...
				// Modificar la cookie al final de la función
				$documentoIdentificacion = $this->input->post('numero_documento'); // Obtener el DPI

				// Obtener la cookie existente
				$cookie = get_cookie('codigo_plaza', TRUE);
				...
				if ($cookie) {
					...
					$cookieData['dpi'] = $documentoIdentificacion; // Actualizar el campo 'dpi'
					$cookieData['solicitud'] = false;             // Cambiar 'solicitud' a false
					$cookieData['pruebas'] = true;                // Cambiar 'pruebas' a true

					// Guardar la cookie actualizada
					$newCookie = array(
						'name' => 'codigo_plaza',
						'value' => json_encode($cookieData), // Convertir a JSON
						'expire' => '86500', // Tiempo de expiración en segundos
